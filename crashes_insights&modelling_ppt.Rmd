...
$$ 
y_i \sim NegBinomial(\mu_i,r) \text{,   where   } log(\mu_i)=\beta_0+\sum_{k=1}^K \beta_kx_{ki} \text{   for i} \in 1...n 
$$

```{r bayes, eval=TRUE,echo=FALSE, alignment="left",message=FALSE,size='tiny'}
...
```{r bayes_summary, eval=TRUE,echo=TRUE, alignment="left",message=FALSE,size='tiny'}
nb_bayes <-
  brms::brm(
...
```{r frequentist, echo=FALSE, alignment="left",message=FALSE,out.width="100%"}
nb_freq <-
...
print("R-square")
as.numeric(with(summary(nb_freq), 1 - deviance / null.deviance))
```
